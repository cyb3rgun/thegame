...
- Hard rule: targets are undead, machines or objects. Never living beings, never humans. **Superseded by D-063, current version below.**
- Target rule, current version (D-063, supersedes the line above): targets are armed offenders, machines or the undead. Never bystanders, never hostages. A law enforcement scenario without human offenders is not a law enforcement scenario; the rule protects the innocent, not the species.
- The product is for adults: not suitable for anyone under 18, developed with an 18+ classification as its target (D-064).
- No binary release is ever published. Whoever wants to run it compiles, configures, sets up and administers it themselves (D-065). The repository is source available, not open source (D-066). **Superseded by D-070, current version below.**
- Licensing, current version (D-070, D-071, supersedes the line above): the code is published under the Business Source License 1.1. Anyone may install, configure, change and operate it, including commercially and for their own guests at their own premises; offering it to third parties as a service or reselling it takes a commercial agreement. Each version becomes GPL-3.0 four years after release. Models, content packs, plugins and updates are available in the shop, and game content is licensed separately in LICENSE-CONTENT.md.
- Content comes in two tiers (D-067). The open tier is code, documentation, configuration, brand files and primitive placeholder geometry; it builds and runs on its own (D-068). The closed tier is characters, finished environments, enemy models, AI personalities, soundtrack and any purchased or licensed asset, available only under a rental and server agreement, and it never enters git history (D-069). See docs/content-tiers.md. **The rental and server agreement is superseded by D-071: game content is available through the shop and through service agreements.**
